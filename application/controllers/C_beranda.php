...
class C_beranda extends CI_Controller {
	...
	function index(){
       ...
       // $this->mdl_home->getsqurity();
        ...
        $data['count_max'] = $this->M_beranda->getCount();
	...
	public function delete_gambar($id) {
		...
		if ($id == 1) {
			...
			unlink(realpath('assets/images/')."/".$res->file_slider1);
		...
		if ($id == 2) {
			...
			unlink(realpath('assets/images/')."/".$res->file_slider2);
		...
		if ($id == 3) {
			...
			unlink(realpath('assets/images/')."/".$res->file_slider3);
		...
		if ($id == 4) {
			...
			unlink(realpath('assets/images/')."/".$res->file_slider4);
		...
		if ($id == 5) {
			...
			unlink(realpath('assets/images/')."/".$res->file_slider5);
	...
	public function update1() {
		...
		$new_name = str_replace(" ", "-", str_replace(".", "", $res->keyword))."-5".time();
		...
		// if (@$this->input->post('file-upload2')) {
			...
			$get_name = $this->upload->data();
	   		...
	   		$data['file_slider5'] = $get_name['file_name'];
		...
		$new_name = str_replace(" ", "-", $res->judul_tentang).time();
		...
		// if (@$this->input->post('file-upload2')) {
 			...
 			@unlink(realpath('assets/images/')."/".$res->foto_tentang);

			$get_name = $this->upload->data();

	   		$data['foto_tentang'] = $get_name['file_name'];
